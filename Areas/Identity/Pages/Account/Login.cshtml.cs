...
namespace TaskManager.Areas.Identity.Pages.Account
{
    ...
    public class LoginModel : PageModel
    {
        ...
        public class InputModel
        {
            ...
            [Display(Name = "Запомнить информацию для входа?")]
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    bool PM = await _userManager.IsInRoleAsync(user, "PROJECT MANAGER");
                    if (PM)
                    {
                        return RedirectToAction("index", "Projects");
                    }
                    bool users= await _userManager.IsInRoleAsync(user, "USER");
                    if (users)
                    {
